...
export function getSupportedChains() {
  return window.metaid.cosmos.request({ method: 'cos_supportedChainNames' }) as Promise<SupportedChainNamesResponse>;
}

export function getActivatedChains() {
  return window.metaid.cosmos.request({ method: 'cos_activatedChainNames' }) as Promise<ActivatedChainNamesResponse>;
}

export function getSupportedChainIds() {
  return window.metaid.cosmos.request({ method: 'cos_supportedChainIds' }) as Promise<SupportedChainIdsResponse>;
}

export function getActivatedChainIds() {
  return window.metaid.cosmos.request({ method: 'cos_activatedChainIds' }) as Promise<ActivatedChainIdsResponse>;
}

export function getAccount(chainName: string) {
  return window.metaid.cosmos.request({ method: 'cos_account', params: { chainName } }) as Promise<AccountResponse>;
}

export function requestAccount(chainName: string) {
  return window.metaid.cosmos.request({ method: 'cos_requestAccount', params: { chainName } }) as Promise<RequestAccountResponse>;
}

export function addChain(chain: AddChainParams) {
  return window.metaid.cosmos.request({ method: 'cos_addChain', params: { ...chain } }) as Promise<boolean>;
}

export function signAmino(chainName: string, doc: SignAminoDoc, options?: SignOptions) {
  return window.metaid.cosmos.request({
...
export function on(eventName: 'accountChanged', handler: () => void) {
  return window.metaid.cosmos.on(eventName, handler);
}

export function off(event: unknown) {
  window.metaid.cosmos.off(event);
}

export function onAccountChanged(handler: () => void) {
  return window.metaid.cosmos.on('accountChanged', handler);
}
